                    ...
                    @foreach (getIsFeaturedCategories() as $category)
                        @if ($count < 4)
                            <div class="categories-item">
                                <img src="{{ file_exists(public_path('uploads/category/thumb/' . $category->image)) ? asset('uploads/category/thumb/' . $category->image) : asset('uploads/category/thumb/null.png') }}"
                                    alt="">
                            ...
                            <ul class="nav--sub-categories">
                                @foreach ($category->getSubCategories as $subcategory)
                                    @if ($subcategory->showHome == 'Yes')
                                        <li><a
                                                href="{{ url('categories', [$category->slug, $subcategory->slug]) }}">{{ $subcategory->name }}</a>
                                        </li>
                                    @endif
                                @endforeach
                            </ul>
