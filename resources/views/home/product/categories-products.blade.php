@if ($category && $category->getSubCategories->isNotEmpty())
    ...
    @foreach ($category->getSubCategories as $subCategory)
        @if ($subCategory->showHome == 'Yes')
            <div class="category-view-{{ $count }} type-{{ $typeNum }}"
                id="category-view-{{ $typeNum }}">
                <div class="container">
                    <div class="wrap">
                        <div class="heading">
                                            ...
                                            <div class="item">
                                                <div class="dot-image">
                                                    ...
                                                    @if ($product->promotion && $product->promotion->value !== null)
                                                        <div class="label">
                                                            <span>-{{ $product->promotion->value * 100 }}%</span></div>
                                                    @endif
                                                </div>
                                                <div class="dot-info">
                                                    <h2 class="dot-title"><a href="">{{ $product->title }}</a>
                                                    </h2>
                                                    <div class="product-price">
                                                        ...
                                                        @else
                                                            <span
                                                                class="current">{{ number_format($product->price, 0, ',', '.') }}
                                                                VND</span>
                                                        @endif
                                                    </div>
                                                </div>
