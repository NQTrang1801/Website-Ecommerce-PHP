                ...
                @foreach ($productsSale as $product)
                    <div>
                        <a href="{{ url('products', $product->id) }}">
                            <img src="{{ file_exists(public_path('uploads/product/products/thumb/' . $product->images->image_1)) ? asset('uploads/product/products/thumb/' . $product->images->image_1) : asset('uploads/product/products/thumb/null.png') }}"
                                alt="">
                        </a>
                        <div>
                            <p class="second-introduce-name">{{ $product->title }}</p>
                            <p class="second-introduce-price">
                                <del>{{ number_format($product->price, 0, ',', '.') }}</del><del>VND</del>
                                <span>{{ number_format($product->price * (1-$product->promotion?->value), 0, ',', '.') }}</span>
                                VND</p>
                        </div>
                    </div>
                @endforeach
